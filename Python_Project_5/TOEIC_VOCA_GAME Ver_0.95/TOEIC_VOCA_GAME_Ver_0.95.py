...
def score():
    ...
    for question, meaning in wrong_answers:
        ...
        if my_score >= 80:
            score_label = Label(win, text=f'{my_score:.0f}' "점입니다. 합격입니다.", font=("Arial", 15, "bold"), bg = BGCOLOR, fg = 'white')
            score_label.place(x = 80, y = 130+num_words*30+30, width = 390, height = 30)
        elif my_score < 80:
            score_label = Label(win, text=f'{my_score:.0f}' "점입니다. 다시 시험 보세요.", font=("Arial", 15, "bold"), bg = BGCOLOR, fg = 'white')
            score_label.place(x = 80, y = 130+num_words*30+30, width = 390, height = 30)
...
# 질문 개수 Label 
lab = Label(win)
lab.config(text = "몇개의 단어를 테스트 할까요?", font = ("Arial", 15, "bold"), bg = BGCOLOR, fg = "white")
